...
### **手順**
> :warning:これはあくまでも一つの作成手法を紹介。プロジェクトに適宜の手法を使いましょう。
...
### **前提** {#dev-前提}
GitHubアカウントがプロジェクトのリポジトリをアクセスできること。

### **手順** {#dev-手順}
> Gitの操作はコマンドを書いていますが、UIやVS Codeのエクステンションで操作もあり。
1. リポジトリをローカルフォルダにcloneする。
```
git clone <リポジトリ>
```
2. 開発機能のブランチにcheckout。
```
git checkout <ブランチ>
```
3. コーディング
4. ソースコミット
> 開発途中のソースをコミットしてもいいですが、通していないロジックやビルドエラーのソースはコミットしないください（常識を持ちましょう）。
> 場合によって先に上流からfetchする。
```
git add <変更したファイル>
git commit -m "<コメント>"
...
> コメントはちゃんと書きましょう。
5. pull requestを作成。
6. 担当のコードレビュアーがレビューする。問題がないの場合は該当ブランチを削除。
ローカルブランチ削除。
```
git branch -D <ブランチ>
```
